    ...
    <div class="orderContainer">
      <div class="orderMsg">
        ...
        <div class="productMsg">
          ...
          <span>|</span>
          <span
            >订单号：<span id="goToDetail" @click="goToDetails">{{
              orderid_
            }}</span></span
          >
          <span>|</span>
      ...
      <div class="currentOrder">
        <div class="products">
          ...
          >
            ...
            <p>
              <span>{{ item.price }}积分</span><span> x </span
              ><span>{{ item.amount }}</span>
            </p>
          </div>
          <!-- 订单信息 -->
          <div class="details" v-show="isshow" v-if="good[0]">
            <div class="orderId">
              <p>订单号：{{ orderid_ }}</p>
            </div>
            <div class="address">
              <p>下单时间：{{ good[0].time }}</p>
            </div>
            <div class="itemMessage">
              <p class="itemname">商品名称：{{ good[0].itemname }}</p>
        ...
        <div class="rightSide">
          <p v-show="totalPrice === 1">立即付款</p>
          <p style="cursor: pointer" @click="changeshow(orderid_)">订单详情</p>
          <p>联系客服</p>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async changeshow(orderid) {
      this.isshow = !this.isshow;
      console.log("orderid_------", this.orderid_);
      try {
        const response = await axios.get(
          `http://localhost:3001/api/dingdan/${orderid}`
        );
        this.good = response.data;
        console.log("changeshow收到30001端口的data:", this.good);
      } catch (error) {
        console.log(error);
      }
    },
